...
read a blob, but it can *hand one to someone who might*. A data subject gets
...
can log in and decrypt nothing. An endpoint that *pretends* to do it is worse
...
- The admin surface is strictly narrower than the user-facing one. It can see
  *less* about a user's data than that user can, by construction.
